...
**Regla (revisada EP-17-b, 2026-07-08):** La aplicación usa una **única cuenta SMTP global**, configurada vía variables de entorno del proceso (`SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURE`, `SMTP_USER`, `SMTP_PASSWORD`, `SMTP_FROM_EMAIL`, ver `apps/server/.env.example` y `src/config/mailConfig.ts`). **Todos los tenants activos** (`isActive: true`) son elegibles para recibir recordatorios automáticos — ya no existe un chequeo de "¿el tenant tiene SMTP propio configurado?" como filtro de elegibilidad. El nombre visible del remitente (`fromName` del mail) sigue siendo el **nombre del negocio** (`tenant.name`), para que el mail se perciba como enviado por la estética aunque el transporte SMTP real sea compartido por toda la app. El envío de recordatorios corre en un cron interno (`node-cron`) que sigue siendo **idempotente** vía un campo de estado en `Appointment`. El único campo de configuración que permanece por-tenant es `Tenant.notificationSettings.reminderHoursBefore` (ventana de anticipación, editable vía `GET`/`PUT /api/notificaciones`).

**Por qué el cambio:** Exigirle a cada centro de estética que configure y mantenga sus propias credenciales SMTP (host, puerto, usuario, contraseña cifrada) era fricción de onboarding sin beneficio proporcional — la mayoría de los tenants nunca completaba esa configuración y por lo tanto nunca recibía recordatorios (ver diagnóstico en `progress/explores/_archive/explore_mail-architecture.md`). Centralizar el SMTP en la app (una sola cuenta transaccional, ej. SendGrid/SES) garantiza entregabilidad consistente y elimina la necesidad de cifrar/almacenar un secreto de terceros por tenant.

**Mandatos:**
1. La config SMTP se lee **exclusivamente** de variables de entorno (`process.env.SMTP_*`), nunca de un documento Mongo. Prohibido reintroducir credenciales SMTP en `Tenant` u otra colección de negocio.
2. `src/config/mailConfig.ts` centraliza la lectura de esas variables y loguea un `console.warn` (no crashea el proceso) si falta alguna crítica al arrancar — útil en entornos de desarrollo sin SMTP configurado.
3. `src/services/mailService.ts` arma el transporter de `nodemailer` **una sola vez a nivel de módulo** (ya no por-llamada, porque la config ya no varía por tenant). El `from` del mail usa `tenant.name` como nombre visible y `mailConfig.fromEmail` como dirección real.
4. El criterio de elegibilidad de `reminderScheduler.ts` es únicamente `Tenant.isActive: true` + tener turnos `pending`/`confirmed` dentro de la ventana `reminderHoursBefore`. `Appointment.reminderSent: boolean` (default `false`) sigue siendo la fuente de verdad de idempotencia — el cron solo procesa turnos con `reminderSent: false`, `isActive: true` y `status` en `['pending', 'confirmed']`.
5. Si el cliente del turno no tiene `email`, el turno se **omite silenciosamente** (no es un error) y no se marca `reminderSent` (para permitir reintento si más adelante se carga el email del cliente).
6. `utils/crypto.ts` (`encryptSecret`/`decryptSecret`, AES-256-GCM) queda sin consumidores tras este cambio — se conserva en el repo por si una feature futura necesita cifrar un secreto en reposo, pero no se usa en el flujo de notificaciones.

**Auditado por:** `CHECKPOINTS.md` C7 (seguridad de credenciales) — verificar que no queden referencias muertas a `smtpPasswordEncrypted`/`decryptSecret` en el flujo de mail, y que `mailConfig.ts` no loguee ni exponga la contraseña SMTP en texto plano.
