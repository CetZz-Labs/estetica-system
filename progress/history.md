...
## 2026-07-28/29 — UX-49 a UX-55: fix del glow de DotField, polish del CTA final, reveal reversible de "Cómo funciona" y página `/guia` (product tour)

* **Agente:** Claude (Leader) + 4 implementers frontend (en 3 rondas) + 2 reviewers (en 3 rondas, una interrumpida por límite de sesión y relanzada).
* **Objetivo:** el usuario pidió (1) una sección/página nueva que muestre todas las funcionalidades del producto con pasos y medios (video/captura), y (2) reportó en vivo, mientras se trabajaba, varios bugs/mejoras puntuales de la Landing pública ya existente (glow del fondo interactivo, reveal de "Cómo funciona", alineación de botones del CTA, blur del CTA, índice sticky y volumen de video de la página nueva). Las 7 features se cerraron en la misma sesión.

* **UX-49 — Fix del glow negro/opaco de `DotField`:** el `<radialGradient>` del glow cerraba con `stopColor="transparent"` (negro con alfa 0 en SVG, no "ausencia de color") y `glowOpacity` no tenía techo, saturando el glow a un disco oscuro con movimientos rápidos del mouse. Fix: 3 stops del mismo `glowColor` variando solo `stopOpacity` (100%→35%→0%) + prop `glowMaxOpacity` (default 0.22) que acota el techo de opacidad. `DOTFIELD_GLOW_COLOR` en `Landing.tsx` pasó de `wine` (#6B3444) a `accent-rose` (#D98BA4). **Incidente de proceso:** la primera ronda del implementer introdujo 2 cambios fuera de alcance sin avisar (opacidad de `AnimatedStat`, default de `gradientFrom`/`gradientTo`) — detectados por el leader comparando `git diff` normal vs. `--ignore-all-space` (gran parte del diff era reindentación espuria). Al pedir el revert, el implementer detectó que el usuario estaba editando `Landing.tsx` a mano en simultáneo (ajustando props de `DotField` y opacidades de cards) y correctamente NO forzó un `Write` que hubiera pisado ese trabajo — se pausó todo implementer sobre `Landing.tsx` hasta que el usuario confirmó que había terminado.
* **UX-51 — "Cómo funciona": reveal reversible:** `viewport={{ once: true, ... }}` → `once: false` en el contenedor de cada paso y en su círculo numerado, para que el reveal se revierta al salir del viewport en cualquier dirección (no solo la primera vez).
* **UX-52 — CTA final: alinear altura de botones:** "Iniciar sesión" tenía `border border-white/30` y "Crear cuenta gratis" no — mismo padding, 2px de diferencia de alto por el borde. Fix: `border border-transparent` en el botón sin borde.
* **UX-53 — CTA final: efecto `GradualBlur`:** nuevo componente `components/landing/GradualBlur.tsx` (puerto local de react-bits, cero dependencias), capas `absolute` con `backdrop-filter: blur()` creciente y `mask-image: linear-gradient(...)` que controla SOLO el canal alfa de cada capa (nunca `background`). Requirió una excepción nueva documentada en `docs/design.md` §13.1 (mismo criterio que el `radialGradient` de un color de DotField: un gradiente en `mask-image` no pinta color, solo modula visibilidad de un blur ya existente).
* **UX-50 — Página `/guia` (tour de producto):** nueva ruta pública `/guia` (`views/Guia.tsx`, fuera de `AppLayout`) con 9 módulos (Login, Panel principal, Clientes, Servicios, Inventario, Registro de visitas, Turnos, Profesionales, Historial — este último redactado a partir de `views/Historial.tsx`, no está en la guía de referencia). Contenido adaptado de `docs/guia_pagina_landing.html` (Maison→Shear, tono de marketing). Contrato de medios de 3 estados (`video`/`image`/`none`) en `components/landing/guide/guideContent.ts`, render en `ModuleMedia.tsx`, índice sticky con `IntersectionObserver` en `GuideIndex.tsx`. El usuario subió 5 videos reales durante la sesión (dashboard, clientes, servicios, inventario, turnos), organizados por el leader en `apps/client/public/media/<slug>/demo.mp4`; los 4 módulos restantes quedan en placeholder a la espera de capturas.
* **UX-54 — Fix: índice de `/guia` no quedaba sticky:** el `<ul lg:sticky lg:top-24>` vivía dentro de un `<nav>` sin altura explícita; el grid de `Guia.tsx` estira el grid item (`align-items: stretch` nativo) pero esa altura no se propaga a descendientes en bloque sin `h-full` — el `<nav>` corto era el *containing block* del sticky, sin recorrido vertical. Fix: `lg:h-full` en el `<nav>`. **Promovido a patrón reutilizable:** `docs/patterns-frontend.md` § P16.
* **UX-55 — Fix: volumen de video muy alto:** sin atributo HTML estático para volumen inicial; fix con `ref` + `onLoadedMetadata` seteando `videoElement.volume = 0.5` por instancia (sin listener global compartido entre los 9 videos).

* **Archivos nuevos:** `components/landing/GradualBlur.tsx`, `components/landing/guide/{guideContent.ts,ModuleMedia.tsx,GuideIndex.tsx}`, `views/Guia.tsx`, `apps/client/public/media/<9 slugs>/` (5 con `demo.mp4` real).
* **Archivos modificados:** `components/landing/DotField.tsx`, `views/Landing.tsx` (nav + 4 fixes puntuales), `router.tsx` (ruta `/guia`), `docs/design.md` (§13.1, excepción `mask-image`), `docs/patterns-frontend.md` (P16 nuevo).
* **ADRs / hallazgos de proceso:**
  - Las 7 features pasaron por `feature_list.json` con más de una en `"in_progress"` simultáneamente (C2 del checklist de reviewer lo marca como límite de paralelismo) — decisión deliberada del leader para agrupar ajustes chicos y no conflictivos del mismo archivo en una sola ronda de implementer/reviewer, documentada como hallazgo no bloqueante en los `review_*.md` correspondientes, no como violación real.
  - Un reviewer se cortó a mitad de camino por límite de sesión de la API (no por hallazgo de código) — se relanzó desde cero sin asumir nada del intento previo; `feature_list.json` y los `review_*.md` no habían sido tocados por el intento fallido.
  - Verificación empírica en las 7: `pnpm --filter @estetica/client build` y `pnpm --filter @estetica/client lint` exit 0 en cada ronda (los 4 warnings de `react-hooks/incompatible-library` son preexistentes, ajenos a esta sesión).
  - **Limitación estándar documentada:** sin navegador real en este entorno — a diferencia de sesiones anteriores, esta vez el usuario sí dio feedback visual real de su propio navegador en vivo (reportó el glow oscuro, el índice no-sticky y el volumen alto), incorporado directamente a la ronda siguiente.
* **Estado final:** `UX-49` a `UX-55` → `"done"`, todas con `progress/reviews/review_<id>.md`. Pendiente (fuera de esta entrada): el usuario subirá capturas de pantalla para los módulos de `/guia` sin video (login, visitas, profesionales, historial).

---

## 2026-07-29 — UX-56 a UX-58: velocidad del hero, íconos del TrustMarquee y capturas de `/guia`

* **Agente:** Claude (Leader) + 3 implementers frontend (1 por feature) + 3 reviewers (1 por feature; el primer intento de reviewer de UX-54/55 en el ciclo anterior se había cortado por límite de sesión de la API y se relanzó sin incidentes — no aplica a este bloque, mencionado por continuidad).
* **Objetivo:** feedback puntual del usuario, en vivo, sobre la Landing y la página `/guia` recién cerradas en el ciclo anterior (UX-49→UX-55).

* **UX-56 — Silk del hero aún más rápido:** el usuario pidió acelerar más la animación de olas del hero. `speed` (multiplicador lineal de `uSpeed * uTime` en el shader) subió de `7` a `22` en el punto de montaje de `views/Landing.tsx` (mismo factor de progresión ~3.2x que la ronda anterior, 2.2→7). Default del componente `Silk.tsx` sin tocar.
* **UX-57 — TrustMarquee con íconos por funcionalidad:** los puntos de color de la cinta de confianza (`marqueeWords`) se reemplazaron por los mismos íconos `react-icons/fi` que ya usa cada funcionalidad en la sección Funcionalidades (FiUsers/FiScissors/FiBox/FiCalendar/FiCheckCircle/FiActivity) — cero imports nuevos, misma rotación de color (`bg-*`→`text-*`, mismos 4 tonos por índice).
* **UX-58 — Capturas reales en `/guia`:** el usuario subió 5 capturas (login, historial, registro de visita, listado de profesionales, alta de profesional). Conectadas en `guideContent.ts` reemplazando los placeholders `{ kind: 'none' }` de los módulos `login`/`historial`/`visitas`/`profesionales`. El módulo `profesionales` necesitaba 2 imágenes para 2 pasos distintos, lo que llevó a extender el contrato: `GuideStep` ganó un campo opcional `media?: ModuleMedia`, renderizado en `Guia.tsx` junto al paso correspondiente (además del `ModuleMedia` de nivel de módulo ya existente); `ModuleMedia.tsx` ganó una prop opcional `inline` para el tamaño más angosto de ese uso dentro de un paso. **Incidente resuelto en el camino:** la primera captura de login subida por el usuario resultó ser la pantalla de "crear cuenta" (registro), no la de inicio de sesión — detectado por el leader antes de conectarla (visualizando el PNG), consultado con el usuario, quien la reemplazó por la captura correcta antes de continuar.
* **Archivos modificados:** `views/Landing.tsx` (UX-56, UX-57), `components/landing/guide/guideContent.ts`, `views/Guia.tsx`, `components/landing/guide/ModuleMedia.tsx` (UX-58).
* **Verificación:** `pnpm --filter @estetica/client build`/`lint` exit 0 en las 3 rondas.
* **Estado final:** `UX-56`, `UX-57`, `UX-58` → `"done"`. **Pendiente:** los 9 módulos de `/guia` ya tienen algún tipo de medio (5 con video, 4 con captura) — sin placeholders `{ kind: 'none' }` restantes. **Nota del reviewer de UX-58, no bloqueante:** hay un volumen considerable de archivos sin commitear en el repo (toda la Landing/`/guia` de este ciclo y el anterior) — sugerido consolidar en un commit cuando el usuario lo indique explícitamente (nunca de forma autónoma).

---

## 2026-07-29 — UX-59: CTA final — legibilidad (quitar blob blanco + puntos) y botones siempre en fila

* **Agente:** Claude (Leader) + 1 implementer frontend + 1 reviewer.
* **Objetivo:** el usuario reportó que el `<HeroBlob bg-white blend soft-light>` montado detrás del texto de la card wine del CTA final aclaraba demasiado el centro y afectaba la legibilidad; pidió también quitar la textura de puntos de fondo (`ctaDotPatternUrl`) y que los botones "Crear cuenta gratis"/"Iniciar sesión" queden siempre en fila horizontal (antes se apilaban en mobile, `flex-col sm:flex-row`).
* **Cambios:** eliminado el mount de `<HeroBlob>` y, al quedar sin ningún otro consumidor en el archivo, se eliminó también la función `HeroBlob`/interfaz `HeroBlobProps` completas (código muerto). Eliminado el `<div>` de textura de puntos y la constante `ctaDotPatternUrl` (sin otro uso). Contenedor de botones: `flex flex-col sm:flex-row` → `flex flex-row flex-wrap` (siempre en fila, con wrap de seguridad en viewports muy angostos); wrappers `Magnetic` de cada botón: `block sm:inline-block w-full sm:w-auto` → `inline-block w-auto` (ancho de contenido en todos los tamaños). El `GradualBlur` del borde inferior (UX-53) no se tocó.
* **Verificación:** `pnpm --filter @estetica/client build`/`lint` exit 0 (el lint confirmó ausencia de warnings de variables/funciones no usadas tras el borrado de código muerto).
* **Estado final:** `UX-59` → `"done"`. **Nota reiterada por el reviewer (2da vez, no bloqueante):** el volumen de trabajo sin commitear en el repo (Landing/`/guia` desde UX-45) empieza a dificultar la auditoría diferencial de features nuevas — el leader debe proponerle al usuario un commit de checkpoint explícito antes de seguir acumulando.

* **Commit de checkpoint (2026-07-29):** a pedido explícito del usuario (tras la 2da recomendación de un reviewer), se commiteó localmente todo el trabajo de UX-49 a UX-59 en `92a722f` — "feat(ui): UX-49..59 — Landing pública: fix DotField/CTA/TrustMarquee + página /guia (product tour)". Sin push.

---

## 2026-07-29 — UX-60 a UX-62: logo 3D del hero, fix de overflow mobile, fix del botón "Iniciar sesión"

* **Agente:** Claude (Leader) + 2 implementers frontend (1 combinado para UX-60/61, 1 para UX-62) + 2 reviewers.
* **Objetivo:** el usuario pidió reemplazar las 3 cards de estadísticas del hero por un render 3D real del logo de Shear (con la restricción de gobernanza explícita de no reabrir three.js/GSAP, retirados en UX-45), reportó overflow horizontal en mobile, y detectó un botón desalineado en el CTA final.
* **UX-60 — Logo 3D del hero:** nuevo `components/landing/HeroLogo3D.tsx`, usando `ogl` (ya instalado, antes exclusivo de `Silk.tsx` — esta feature extiende el permiso documentado en `docs/design.md` §13.1). `Camera` con perspectiva real + `Transform` + `Mesh`(`Plane`)/`Program` con `Texture` de `/shear-favicon.png`, rotación continua + sombreado difuso/especular simple, `prefersReducedMotion` con frame estático, cleanup de 4 pasos igual que `Silk.tsx`. **Iteración intermedia:** el usuario pidió más profundidad en Z; el implementer probó una geometría tipo caja, el usuario la vio "extremadamente mal" en su navegador y pidió revertir — se restauró el `Plane` simple original (confirmado por hash de build idéntico). `heroStatCards` eliminado junto con el import de ícono que quedó sin uso; los imports/componentes compartidos con Stats/Features (`AnimatedStatIcon`, `sectionTints`, `TiltCard`) se dejaron intactos tras confirmar con grep que siguen en uso.
* **UX-61 — Fix overflow horizontal mobile:** diagnóstico del leader (sin sub-agente de exploración, por lectura directa de código): NO era el menú mobile (solo se monta cuando está abierto). La causa real era `<section id="como-funciona">`, sin ningún `overflow-hidden`, cuyos pasos usan `x: ±140px` como estado oculto (UX-48) — al volverse reversible en UX-51 (`once: false`, esta misma sesión), ese estado transformado fuera de pantalla pasó a repetirse constantemente al scrollear en vez de ocurrir una sola vez. Fix: `overflow-x-hidden` en esa sección. Confirmado por grep que era el único transform horizontal sin contener en el archivo.
* **UX-62 — Fix botón "Iniciar sesión":** el usuario detectó (inspeccionando el DOM) que el botón quedaba corrido hacia arriba. Causa: era el único de los 2 botones del CTA final sin `flex items-center justify-center` (bug preexistente desde UX-45, invisible hasta que UX-59 acercó los botones al quitarles `w-full`). Fix de una clase.
* **Archivos:** `components/landing/HeroLogo3D.tsx` (nuevo), `views/Landing.tsx`, `docs/design.md` (§13.1).
* **Verificación:** `pnpm --filter @estetica/client build`/`lint` exit 0 en ambas rondas.
* **Estado final:** `UX-60`, `UX-61`, `UX-62` → `"done"`. **Pendiente anunciado por el usuario:** va a agregar un SVG vectorial del logo (`apps/client/public/media/shear-favicon.svg`) para intentar una extrusión 3D más fiel a la silueta real en una próxima ronda (UX-63).

---

## 2026-07-29 — UX-63/UX-64: extrusión 3D real del logo del hero (silueta exacta del SVG, ogl+earcut)

* **Agente:** Claude (Leader) + 1 implementer frontend (mismo agente para ambas rondas, continuado vía mensaje) + 2 reviewers.
* **Objetivo:** el usuario subió el SVG vectorial real del logo (`apps/client/public/media/shear-favicon.svg`, 4 `<path>` de distinto color, viewBox 489×483) para reemplazar la aproximación de plano simple de `HeroLogo3D` (UX-60) por una extrusión 3D que siga la silueta exacta, tras el rechazo de un intento anterior con caja genérica.
* **UX-63 — Extrusión inicial:** nuevo `components/landing/svgExtrude.ts` — parser de path SVG a mano (M/L/C/Z, aplanado de curvas Bézier cúbicas), clasificación isla/hueco por paridad de profundidad de anidamiento (replica `fill-rule="evenodd"` del SVG fuente), triangulación por isla con `earcut` (nueva dependencia, única aprobada explícitamente por el usuario tras una pregunta de clarificación sobre el trade-off precisión-vs-riesgo), generación de paredes laterales con normal saliente calculada analíticamente. `HeroLogo3D.tsx` pasó de `Plane` a esta geometría extruida (profundidad inicial `0.22`), con carga async del SVG vía `fetch` + `DOMParser` y guarda `cancelled` contra fugas tras desmontar. El reviewer verificó empíricamente (no solo por lectura) el riesgo de winding señalado por el implementer, ejecutando la función de geometría de forma aislada: 4380/4432 triángulos con área con signo positiva, confirmando que no hacía falta invertir el orden de índices de la cara frontal.
* **UX-64 — Fix de seguimiento:** el usuario probó UX-63 en su navegador (tras un fix aparte de caché de Vite, `--force`, por el error "Outdated Optimize Dep" al agregar `earcut` con el dev server corriendo) y reportó 2 problemas: profundidad exagerada, y partes del ícono faltantes. Causa de lo segundo: `HeroLogo3D.tsx` solo leía el primer `<path>` del SVG (`querySelector`), pero los otros 3 paths tienen geometría propia no contenida en la silueta del primero. Fix: `EXTRUDE_DEPTH` 0.22→0.08; nueva `buildExtrudedLogoGeometryFromPaths` en `svgExtrude.ts` que procesa cada uno de los 4 paths con su propia pasada independiente de clasificación isla/hueco (evitando clasificación cruzada semánticamente incorrecta entre paths distintos) y concatena los 4 buffers no-indexados con `mergeExtrudedGeometry`.
* **Archivos:** `components/landing/svgExtrude.ts` (nuevo), `components/landing/HeroLogo3D.tsx`, `apps/client/package.json`/`pnpm-lock.yaml` (+earcut), `docs/design.md` (§13.1).
* **Verificación:** `pnpm --filter @estetica/client build`/`lint` exit 0 en ambas rondas.
* **Estado final:** `UX-63`, `UX-64` → `"done"`. Usuario confirmó visualmente "se ve bien" tras UX-64 — cierra el ciclo de iteración del logo 3D sin necesidad de una tercera ronda.

---

## 2026-07-29 — UX-65: cambio de color del wash Silk del hero

* **Agente:** Claude (Leader) + 1 implementer frontend + 1 reviewer.
* **Objetivo:** el usuario planteó que el color del fondo del hero (wash `Silk`, tono `accent-rose`) podía competir visualmente con el logo 3D (paleta rosa/negro/marrón) y el texto — pidió una combinación de color distinta, sin tocar el estilo general. El leader propuso `sage` (familia de color distinta a la cálida del logo, ya usada como token de marca en otras partes de la Landing) y el usuario no objetó el criterio.
* **Cambio:** `SILK_COLOR` en `views/Landing.tsx` de `'#D98BA4'` (accent-rose) a `'#8C9178'` (sage, `docs/design.md` §2.3), con su comentario explicativo actualizado. Ningún otro prop de `<Silk />` ni `--bg`/`bg-bg` tocados.
